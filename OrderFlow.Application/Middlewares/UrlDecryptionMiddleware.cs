...
namespace OrderFlow.Application.Middlewares
{
    public class UrlDecryptionMiddleware
    {
        ...
        {
            // 1. Проверяем, есть ли в пакете зашифрованный заголовок пути
            if (context.Request.Headers.TryGetValue("X-Encrypted-Path", out var encryptedPathHeader))
            {
                try
                {
                    // 2. Расшифровываем реальный URL-путь (например, "/UserRegister/Register?type=admin")
                    string decryptedUrl = DecryptString(encryptedPathHeader);

                    // Разделяем путь и Query-параметры (если они есть)
                    string[] urlParts = decryptedUrl.Split('?');

                    // ПОДМЕНЯЕМ HTTP-контекст для приложения
                    ...
                    // 3. Расшифровываем тело запроса (Body), если оно передано
                    if (context.Request.ContentType != null && context.Request.ContentType.Contains("application/json"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(bodyJson))
                        {
                            ...
                            if (doc.RootElement.TryGetProperty("payload", out var payloadProp))
                            {
                                ...
                                // Подменяем поток тела запроса на чистый JSON для контроллеров
            ...
            // Передаем управление дальше. Контроллер (например, LoginController) 
            // получит правильный HTTP-запрос с правильным Path и Query!
            await _next(context);
        }
